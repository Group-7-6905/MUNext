...
if (isset($_POST['action'])) {
    ...
    if ($jobid > 0) {
        switch ($action) {
            case 'suspend':
                ...
                if (mysqli_stmt_execute($stmt)) {
                    ...
                    $logQuery = "INSERT INTO tbl_activity_log (USERID, ACTION, DESCRIPTION, IP_ADDRESS, CREATED_AT) 
                                VALUES (?, 'JOB_SUSPENDED', 'Admin suspended an active job posting', ?, NOW())";
            ...
            case 'close':
                ...
                if (mysqli_stmt_execute($stmt)) {
                    // Log activity
                    $logQuery = "INSERT INTO tbl_activity_log (USERID, ACTION, DESCRIPTION, IP_ADDRESS, CREATED_AT) 
                                VALUES (?, 'JOB_CLOSED', 'Admin closed an active job posting', ?, NOW())";
                    $logStmt = mysqli_prepare($con, $logQuery);
                    $ip = getClientIP();
                    mysqli_stmt_bind_param($logStmt, "is", $session_id, $ip);
                    mysqli_stmt_execute($logStmt);
            ...
            case 'delete':
                ...
                if (mysqli_stmt_execute($stmt)) {
                    // Log activity
                    $logQuery = "INSERT INTO tbl_activity_log (USERID, ACTION, DESCRIPTION, IP_ADDRESS, CREATED_AT) 
                                VALUES (?, 'JOB_DELETED', 'Admin deleted an active job posting', ?, NOW())";
                    $logStmt = mysqli_prepare($con, $logQuery);
                    $ip = getClientIP();
                    mysqli_stmt_bind_param($logStmt, "is", $session_id, $ip);
                    mysqli_stmt_execute($logStmt);
        ...
        header('location: admin-active-jobs.php?' . http_build_query($_GET));
        exit();
    }
}
// ==================== END JOB ACTIONS ====================
...
// ==================== BUILD QUERY ====================
$where_clauses = ["j.JOBSTATUS = 'Active'"];
$params = [];
$types = '';
...
if (!empty($filter_type)) {
    $where_clauses[] = "j.EMPLOYMENTTYPE = ?";
    $params[] = $filter_type;
    $types .= 's';
}
...
$query = "SELECT j.*, c.COMPANYNAME, c.COMPANYLOGO,
          ...
          (SELECT COUNT(*) FROM tblbookmarkjob WHERE JOBID = j.JOBID) as BOOKMARK_COUNT
          FROM tbljob j 
          LEFT JOIN tblcompany c ON j.COMPANYID = c.COMPANYID
          $where_sql 
...
// ==================== GET STATISTICS ====================
$totalActiveQuery = "SELECT COUNT(*) as total FROM tbljob WHERE JOBSTATUS = 'Active'";
$totalActive = mysqli_fetch_assoc(mysqli_query($con, $totalActiveQuery))['total'];

$expiringQuery = "SELECT COUNT(*) as total FROM tbljob WHERE JOBSTATUS = 'Active' AND DEADLINE <= DATE_ADD(CURDATE(), INTERVAL 7 DAY) AND DEADLINE >= CURDATE()";
$expiring = mysqli_fetch_assoc(mysqli_query($con, $expiringQuery))['total'];

$highApplicationsQuery = "SELECT COUNT(*) as total FROM tbljob j WHERE j.JOBSTATUS = 'Active' AND (SELECT COUNT(*) FROM tbljobapplication WHERE JOBID = j.JOBID) >= 10";
$highApplications = mysqli_fetch_assoc(mysqli_query($con, $highApplicationsQuery))['total'];

$totalApplicationsQuery = "SELECT COUNT(*) as total FROM tbljobapplication ja INNER JOIN tbljob j ON ja.JOBID = j.JOBID WHERE j.JOBSTATUS = 'Active'";
...
<head>
    ...
    <title>Active Jobs - Admin Dashboard | MUNext</title>
                ...
                <div class="container-fluid">
                    ...
                    <div class="stats-grid">
                        <div class="stat-card total">
                            <span class="stat-number"><?php echo number_format($totalActive); ?></span>
                            <span class="stat-label">Total Active Jobs</span>
                        </div>
                        <div class="stat-card expiring">
                            <span class="stat-number"><?php echo number_format($expiring); ?></span>
                            <span class="stat-label">Expiring in 7 Days</span>
                        </div>
                        <div class="stat-card popular">
                            <span class="stat-number"><?php echo number_format($highApplications); ?></span>
                            <span class="stat-label">High Applications (10+)</span>
                        </div>
                    ...
                    <div class="filter-section">
                        <div class="filter-header">
                            <h6 class="filter-title">
                                <i class="lni lni-funnel mr-2"></i>Filter Active Jobs
                            </h6>
                        </div>
                        ...
                        <form method="GET" action="">
                                ...
                                <div class="col-md-2 mb-3">
                                    ...
                                    <select name="type" class="form-control">
                                        ...
                                        <option value="Full-Time"
                                            <?php echo $filter_type === 'Full-Time' ? 'selected' : ''; ?>>Full-Time
                                        </option>
                                        <option value="Part-Time"
                                            <?php echo $filter_type === 'Part-Time' ? 'selected' : ''; ?>>Part-Time
                                        </option>
                                        <option value="Contract"
                                            <?php echo $filter_type === 'Contract' ? 'selected' : ''; ?>>Contract
                                        </option>
                                        <option value="Internship"
                                            <?php echo $filter_type === 'Internship' ? 'selected' : ''; ?>>Internship
                                        </option>
                                    </select>
                                </div>
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <a href="admin-active-jobs.php" class="btn-reset">
                    ...
                    <div class="jobs-table-card">
                        <div class="table-header">
                            <i class="lni lni-checkmark-circle mr-2"></i>Active Jobs List
                            <span style="float: right; font-weight: normal;">
                                Total: <?php echo number_format($total_records); ?> active jobs
                            </span>
                        </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="action-buttons">
                                                ...
                                                <form method="POST" style="display: inline;"
                                                    onsubmit="return confirm('Suspend this job posting?')">
                                                    <input type="hidden" name="jobid"
                                                        value="<?php echo $job['JOBID']; ?>">
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header" style="background: var(--success-color); color: white;">
                    <h5 class="modal-title">Active Job Details</h5>
